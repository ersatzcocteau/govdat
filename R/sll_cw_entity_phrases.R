...
function(entity,phrase=NULL,mincount=1,sort="count",state=NULL,
         party=NULL,chamber=NULL,start_date=NULL,end_date=NULL,
  key = getOption("SunlightLabsKey", stop("need an API key for Sunlight Labs")))
{
  url = paste("http://capitolwords.org/api/phrases/",entity,".json",sep="")
  page = 0
  args <- compact(list(apikey=key, phrase=phrase,page=page,state=state,party=party,chamber=chamber,start_date=start_date,end_date=end_date))
